...
**Status:** V63 — Eukaryotic UniBin + **`certification/`** organelle + **`composition/`** (`HealthCompositionContext` / primalSpring **CompositionContext**) + **`validation/scenarios/`** (16 scenarios / 8 tracks). IPC-first library defaults (`default = []`, **`barracuda-lib`** opt-in). guideStone Level 5 lineage via **`healthspring_unibin certify`**. **V63 deep debt sweep**: 4 domain param structs (`DosingRegimen`, `PopulationPkVariability`, `ToxicityModelParams`, `AntibioticSimConfig`) replace multi-parameter function signatures; `wire_prefix` + socket-path centralization; Foundation Thread 3 (Immunology/Drug Discovery) seeded. **999 tests**, 95 experiments (83 science + 12 composition Tier 3–5), primalSpring **v0.9.25**, 53 Python baseline scripts + 53 paired `.ipynb` notebooks, 95+ provenance entries (100% coverage). ecoBin 0.9.0. barraCuda v0.3.13. Zero clippy, zero unsafe.
