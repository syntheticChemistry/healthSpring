...
**Status**: V24 — `UniBin`-compliant `healthspring_primal` binary (clap subcommands, SIGTERM handling). Capability-based primal discovery (zero hardcoded names). Songbird announcement wired into startup. Niche deploy graph ready. Three-tier data fetch (biomeOS → NestGate → local) implemented. Next: activate Nest Atomic locally.
