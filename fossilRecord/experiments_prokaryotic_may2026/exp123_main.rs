...
#![allow(
    deprecated,
    reason = "prokaryotic experiment, will be absorbed into validation scenarios"
)]
...
fn main() {
    let mut h = ValidationHarness::new("Exp123: NUCLEUS Composition Parity (healthSpring niche)");
...
fn validate_niche_parity(h: &mut ValidationHarness) {
    ...
    let local_shannon = healthspring_barracuda::math_dispatch::shannon_from_frequencies(&counts);
