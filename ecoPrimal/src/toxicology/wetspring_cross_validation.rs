...
mod tests {
    ...
    fn cross_validate_low_dose_hormetic_zone() {
        for dose in [0.1, 0.5, 1.0, 2.0, 5.0] {
            ...
            assert!(
                hs > SHARED_BASELINE,
                "low dose should be hormetic: dose={dose}, R={hs}"
            );
        }
    }
    ...
    fn cross_validate_toxic_zone() {
        for dose in [100.0, 200.0, 500.0] {
            ...
            assert!(
                hs < SHARED_BASELINE,
                "high dose should be toxic: dose={dose}, R={hs}"
            );
        }
    }
    ...
    fn cross_validate_hormetic_optimum_location() {
        ...
        assert!(
            opt_dose < SHARED_IC50 / 2.0,
            "optimum well below IC50: {opt_dose}"
        );
    ...
    fn biphasic_curve_monotonic_in_hormetic_zone() {
        ...
        assert!(
            found_peak,
            "curve should have a single peak (biphasic shape)"
        );
    }
}
