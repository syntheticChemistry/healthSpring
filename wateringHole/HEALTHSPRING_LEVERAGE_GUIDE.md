...
**Date**: May 12, 2026
**Primal**: healthSpring V64 (`healthspring-barracuda` 0.1.0, ecoBin 0.9.0, guideStone Level 5 via **`healthspring_unibin certify`**, primalSpring **v0.9.25**)
...
Top capabilities by domain (58 science + 30 infra = 88):
...
|---------|------|---------|
| V64 | May 12, 2026 | `validate_ltee_b5` Rust binary (LTEE B5 Tier 0+1, 8/8); `--format json` on validation binaries (projectNUCLEUS Tier 2); `s_toxicology` scenario (17th); Foundation Threads 3+5+8 expressions wired → active; dead `npu` feature removed; exp primal-name centralization. |
