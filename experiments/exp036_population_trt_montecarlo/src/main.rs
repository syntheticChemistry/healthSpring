...
fn main() {
    ...
    let vd_arr: Vec<f64> = (0..n_patients).map(|i| {
        let z = -3.0 + 6.0 * (i as f64) / ((n_patients - 1) as f64);
        (mu_vd + sig_vd * z).exp()
    }).collect();

    let ka_arr: Vec<f64> = (0..n_patients).map(|i| {
        let phase = if i % 2 == 0 { i / 2 } else { n_patients - 1 - i / 2 };
        let z = -2.0 + 4.0 * (phase as f64) / ((n_patients - 1) as f64);
        (mu_ka + sig_ka * z).exp()
    }).collect();

    let ke_arr: Vec<f64> = (0..n_patients).map(|i| {
        let z = 3.0 - 6.0 * (i as f64) / ((n_patients - 1) as f64);
        (mu_ke + sig_ke * z).exp()
    }).collect();
    ...
    for i in 0..n_patients {
        let concs = pkpd::pk_multiple_dose(
            |t| endocrine::pk_im_depot(tc::DOSE_WEEKLY_MG, tc::F_IM, vd_arr[i], ka_arr[i], ke_arr[i], t),
        ...
        trough_arr[i] = times.iter().zip(concs.iter())
    ...
    println!("\n--- Check 1: All PK params positive ---");
    if vd_arr.iter().all(|&v| v > 0.0) && ka_arr.iter().all(|&v| v > 0.0) && ke_arr.iter().all(|&v| v > 0.0) {
        println!("  [PASS]"); passed += 1;
    ...
    let vd_err = (vd_med - pop_trt::VD_TYPICAL).abs() / pop_trt::VD_TYPICAL;
    if vd_err < 0.05 { println!("  [PASS] Vd median={vd_med:.2}"); passed += 1; }
    else { println!("  [FAIL] err={vd_err:.3}"); failed += 1; }

    // --- Check 3: All Cmax > 0 ---
    println!("\n--- Check 3: All Cmax > 0 ---");
    if cmax_arr.iter().all(|&c| c > 0.0) { println!("  [PASS]"); passed += 1; }
    else { println!("  [FAIL]"); failed += 1; }

    // --- Check 4: All AUC > 0 ---
    println!("\n--- Check 4: All AUC > 0 ---");
    if auc_arr.iter().all(|&a| a > 0.0) { println!("  [PASS]"); passed += 1; }
    else { println!("  [FAIL]"); failed += 1; }

    // --- Check 5: AUC has variability ---
    println!("\n--- Check 5: AUC variability ---");
    let auc_mean: f64 = auc_arr.iter().sum::<f64>() / n_patients as f64;
    let auc_var: f64 = auc_arr.iter().map(|&a| (a - auc_mean).powi(2)).sum::<f64>() / n_patients as f64;
    let auc_cv = auc_var.sqrt() / auc_mean;
    if auc_cv > 0.15 { println!("  [PASS] CV={auc_cv:.3}"); passed += 1; }
    else { println!("  [FAIL] CV={auc_cv:.3}"); failed += 1; }

    // --- Check 6: Analytical AUC = F*D/(Vd*ke) relationship ---
    println!("\n--- Check 6: Analytical AUC inversely proportional to ke*Vd ---");
    let ke_vd_products: Vec<f64> = ke_arr.iter().zip(vd_arr.iter()).map(|(&k, &v)| k * v).collect();
    // Patients with higher ke*Vd should have lower AUC (analytical: AUC ∝ 1/(ke*Vd))
    let mid = n_patients / 2;
    let mut paired: Vec<(f64, f64)> = ke_vd_products.iter().zip(auc_arr.iter()).map(|(&kv, &a)| (kv, a)).collect();
    paired.sort_by(|a, b| a.0.partial_cmp(&b.0).unwrap());
    let low_kv_auc: f64 = paired[..mid].iter().map(|(_, a)| a).sum::<f64>() / mid as f64;
    let high_kv_auc: f64 = paired[mid..].iter().map(|(_, a)| a).sum::<f64>() / (n_patients - mid) as f64;
    if low_kv_auc > high_kv_auc {
        println!("  [PASS] low ke·Vd AUC={low_kv_auc:.2} > high ke·Vd={high_kv_auc:.2}");
        passed += 1;
    } else { println!("  [FAIL] low={low_kv_auc:.2}, high={high_kv_auc:.2}"); failed += 1; }

    // --- Check 7: Age-adjusted T0 declines ---
    println!("\n--- Check 7: Age-adjusted T0 declines ---");
    if t0_adj[0] > t0_adj[n_patients - 1] {
        println!("  [PASS] T0(40)={:.1} > T0(75)={:.1}", t0_adj[0], t0_adj[n_patients - 1]);
        passed += 1;
    } else { println!("  [FAIL]"); failed += 1; }
    ...
    if p5 < p50 && p50 < p95 { println!("  [PASS] P5={p5:.2} < P50={p50:.2} < P95={p95:.2}"); passed += 1; }
    else { println!("  [FAIL]"); failed += 1; }

    // --- Check 9: Cohort size ---
    println!("\n--- Check 9: Cohort size ---");
    if cmax_arr.len() == n_patients { println!("  [PASS] {n_patients}"); passed += 1; }
    else { println!("  [FAIL]"); failed += 1; }

    // --- Check 10: Trough levels positive ---
    println!("\n--- Check 10: Trough levels > 0 ---");
    if trough_arr.iter().all(|&t| t > 0.0) { println!("  [PASS]"); passed += 1; }
    else { println!("  [FAIL]"); failed += 1; }
