...
fn main() {
    let mut h = ValidationHarness::new("exp078_antibiotic_perturbation");
    ...
    h.check_bool(&format!("nadir = {nadir:.4} < {h0}"), nadir < h0);
    ...
    h.check_bool("all Shannon values positive", all_positive);
