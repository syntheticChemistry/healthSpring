...
fn main() {
    ...
    if (sum_macro - sum_micro).abs() < tolerances::TWO_COMPARTMENT_IDENTITY {
    ...
    if (prod_macro - prod_micro).abs() < tolerances::TWO_COMPARTMENT_IDENTITY {
    ...
    if (c_at_0 - EXPECTED_C0).abs() < tolerances::TWO_COMPARTMENT_IDENTITY {
    ...
    if rel_err < tolerances::AUC_TRAPEZOIDAL {
    ...
    if (a_coeff + b_coeff - c0).abs() < tolerances::TWO_COMPARTMENT_IDENTITY {
    ...
    if let Some(slope) = terminal_slope(&times, &c_curve, 8.0) {
        ...
        if slope_err < tolerances::TERMINAL_SLOPE {
            println!(
    ...
    let max_diff: f64 = c_two
        ...
        .map(|(a, b)| (a - b).abs())
        .fold(0.0, f64::max);
    if max_diff < tolerances::CPU_PARITY {
    ...
    if (v2 - EXPECTED_V2_L).abs() < tolerances::TWO_COMPARTMENT_IDENTITY && v2 > 0.0 {
    ...
    print!("\n--- Check 14: Vss = V1 + V2 --- ");
    let vss = V1_L + v2;
    if (vss - EXPECTED_VSS_L).abs() < tolerances::TWO_COMPARTMENT_IDENTITY && vss > V1_L {
    ...
    if (cl - cl_from_auc).abs() < tolerances::TWO_COMPARTMENT_IDENTITY {
