...
use healthspring_barracuda::comparative::canine::{
    canine_jak_ic50_panel, human_jak_reference_panels, JakIc50Panel,
};
...
fn main() {
    ...
    let hill_at_ic50_all: Vec<f64> = [&oclacitinib, &human_panels[0], &human_panels[1], &human_panels[2]]
        .iter()
    ...
    h.check_abs("Hill dose-response at C=0: R = 0.0", r_zero, 0.0, MACHINE_EPSILON);

    // Check 12: pathway_selectivity_score: oclacitinib > baricitinib for JAK1 target
    let ocla_off = [oclacitinib.jak2_nm, oclacitinib.jak3_nm, oclacitinib.tyk2_nm];
    let baricitinib = &human_panels[2];
    let bari_off = [baricitinib.jak2_nm, baricitinib.jak3_nm, baricitinib.tyk2_nm];
    ...
    h.check_abs("selectivity_index(10, 1000) = 100", si, 100.0, JAK1_SELECTIVITY);
    ...
    let all_have_four = human_panels.iter().all(|p| {
        [p.jak1_nm, p.jak2_nm, p.jak3_nm, p.tyk2_nm]
            .iter()
            .all(|x| x.is_finite())
    }) && [oclacitinib.jak1_nm, oclacitinib.jak2_nm, oclacitinib.jak3_nm, oclacitinib.tyk2_nm]
        .iter()
        .all(|x| x.is_finite());
