...
fn channel_type(ch: &DataChannel) -> &'static str {
    match ch {
...
fn validate_scenario(
    ...
    check!(*passed, *failed, &format!("{name}: no duplicate node IDs"),
        node_ids.len() == nodes.len());

    for node in nodes {
        check!(*passed, *failed, &format!("{name}/{}: health ≤ 100", node.id),
            node.health <= 100);
        check!(*passed, *failed, &format!("{name}/{}: has channels", node.id),
    ...
    check!(*passed, *failed, &format!("{name}: valid JSON"), val.is_object());
    check!(*passed, *failed, &format!("{name}: has edges array"), val["edges"].is_array());
}
...
fn report_channel_stats(scenario: &HealthScenario) {
    ...
    println!("  Total: {total} channels across {} nodes", scenario.ecosystem.primals.len());
...
fn main() {
    ...
    println!("\n--- NLME Specific Checks ---");
    let nlme_ids: HashSet<&str> = nlme.ecosystem.primals.iter().map(|n| n.id.as_str()).collect();
    check!(passed, failed, "NLME has nlme_population", nlme_ids.contains("nlme_population"));
    check!(passed, failed, "NLME has nca_metrics", nlme_ids.contains("nca_metrics"));
    check!(passed, failed, "NLME has cwres_diagnostics", nlme_ids.contains("cwres_diagnostics"));
    check!(passed, failed, "NLME has vpc_check", nlme_ids.contains("vpc_check"));
    check!(passed, failed, "NLME has gof_fit", nlme_ids.contains("gof_fit"));

    // Biosignal WFDB check
    println!("\n--- WFDB Biosignal Check ---");
    let bio_ids: HashSet<&str> = bio.ecosystem.primals.iter().map(|n| n.id.as_str()).collect();
    check!(passed, failed, "Biosignal has wfdb_ecg", bio_ids.contains("wfdb_ecg"));

    let wfdb_node = bio.ecosystem.primals.iter().find(|n| n.id == "wfdb_ecg").unwrap();
    check!(passed, failed, "wfdb_ecg has TimeSeries",
        wfdb_node.data_channels.iter().any(|ch| matches!(ch, DataChannel::TimeSeries { .. })));
    check!(passed, failed, "wfdb_ecg has Bar (beat types)",
        wfdb_node.data_channels.iter().any(|ch| matches!(ch, DataChannel::Bar { .. })));
    check!(passed, failed, "wfdb_ecg has Gauge",
        wfdb_node.data_channels.iter().any(|ch| matches!(ch, DataChannel::Gauge { .. })));
    ...
    check!(passed, failed,
        &format!("full: 28 nodes (6+4+5+8+5), got {}", full.ecosystem.primals.len()),
    ...
    let has_all_types =
        full_json.contains("\"channel_type\": \"timeseries\"")
    ...
    check!(passed, failed, "full: all 7 DataChannel types present", has_all_types);

    // Cross-track NLME edge
    let edge_pairs: HashSet<(String, String)> = full_e.iter()
        .map(|e| (e.from.clone(), e.to.clone()))
        .collect();
    check!(passed, failed, "cross-track: pop_pk -> nlme_population",
    ...
    write("nlme_study.json", &scenarios::scenario_with_edges_json(&nlme, &nlme_e));
    write("pkpd_study.json", &scenarios::scenario_with_edges_json(&pkpd, &pkpd_e));
    write("microbiome_study.json", &scenarios::scenario_with_edges_json(&micro, &micro_e));
    write("biosignal_study.json", &scenarios::scenario_with_edges_json(&bio, &bio_e));
    write("endocrine_study.json", &scenarios::scenario_with_edges_json(&endo, &endo_e));
    ...
    // Summary
    println!("\n====================================");
    println!("Exp076 Full Pipeline: {passed}/{} checks passed", passed + failed);
    if failed > 0 {
