...
fn main() {
    ...
    if ratio > 0.95 {
        println!("  [PASS] C(5t½)/C_ss = {ratio:.4}");
        passed += 1;
    } else {
        println!("  [FAIL] ratio = {ratio:.4}");
        failed += 1;
    }
    ...
    if cv < 0.05 {
        println!("  [PASS] Plateau CV = {cv:.4}");
        passed += 1;
    } else {
        println!("  [FAIL] CV = {cv:.4}");
        failed += 1;
    }
    ...
    if mean_p > 0.0 {
        println!("  [PASS] Mean plateau = {mean_p:.4}");
        passed += 1;
    } else {
        println!("  [FAIL]");
        failed += 1;
    ...
    if c_post < c_end * 0.50 {
        println!("  [PASS] C(end)={c_end:.4}, C(6mo)={c_post:.4}");
        passed += 1;
    } else {
        println!("  [FAIL]");
        failed += 1;
    ...
    if let Some(th) = t_half_obs {
        if (th - tc::T_HALF_DAYS).abs() / tc::T_HALF_DAYS < 0.15 {
            println!("  [PASS] t½ = {th:.2} days");
            passed += 1;
        } else {
            println!("  [FAIL] t½ = {th:.2}");
            failed += 1;
    ...
    if fluct < 0.10 {
        println!("  [PASS] fluctuation = {fluct:.4}");
        passed += 1;
    } else {
        println!("  [FAIL] fluctuation = {fluct:.4}");
        failed += 1;
    }
    ...
    if rel < 0.10 {
        println!("  [PASS] AUC num={auc:.2}, ana={auc_ana:.2} (err={rel:.3})");
        passed += 1;
    } else {
        println!("  [FAIL] err={rel:.4}");
        failed += 1;
    }
    ...
    if (ratio_c - ratio_d).abs() / ratio_d < 0.01 {
        println!("  [PASS] ratio={ratio_c:.4}, expected={ratio_d:.4}");
        passed += 1;
    } else {
        println!("  [FAIL] ratio={ratio_c:.4}");
        failed += 1;
    }
