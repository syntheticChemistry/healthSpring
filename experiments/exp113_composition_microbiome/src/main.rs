...
fn main() {
    ...
    if let Some(ipc_val) = dispatched {
        h.check_abs("Shannon IPC parity", ipc_val, direct_shannon, tolerances::DETERMINISM);
    } else {
        h.check_bool("Shannon dispatch returned result", false);
    }

    // ── Shannon uniform (analytical: ln(4)) ─────────────────────────
    ...
    if let Some(ipc_val) = dispatched_sim {
        h.check_abs("Simpson IPC parity", ipc_val, direct_simpson, tolerances::DETERMINISM);
    } else {
        h.check_bool("Simpson dispatch returned result", false);
    }

    // ── Pielou: direct vs dispatched ────────────────────────────────
    ...
    if let Some(ipc_val) = dispatched_pie {
        h.check_abs("Pielou IPC parity", ipc_val, direct_pielou, tolerances::DETERMINISM);
    } else {
        h.check_bool("Pielou dispatch returned result", false);
    }

    // ── Chao1: direct vs dispatched ─────────────────────────────────
    ...
    if let Some(ipc_val) = dispatched_chao {
        h.check_abs("Chao1 IPC parity", ipc_val, direct_chao1, tolerances::DETERMINISM);
    } else {
        h.check_bool("Chao1 dispatch returned result", false);
    }

    // ── Colonization resistance: direct vs dispatched ───────────────
    ...
    if let Some(ipc_val) = dispatched_cr {
        h.check_abs("Colonization IPC parity", ipc_val, direct_cr, tolerances::DETERMINISM);
    } else {
        h.check_bool("Colonization dispatch returned result", false);
    }

    // ── Anderson gut: direct vs dispatched ──────────────────────────
    ...
    if let (Some(a), Some(b)) = (r1, r2) {
        h.check_abs("Shannon dispatch determinism", a, b, tolerances::DETERMINISM);
    } else {
