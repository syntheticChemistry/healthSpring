...
fn validate_simpson(passed: &mut u32, failed: &mut u32) {
...
fn validate_auc_and_bray_curtis(passed: &mut u32, failed: &mut u32) {
    ...
    check!(passed, failed, "cpu_reference_complete", true);
}
...
fn main() {
    ...
    validate_simpson(&mut passed, &mut failed);
    validate_auc_and_bray_curtis(&mut passed, &mut failed);
