...
fn main() {
    let mut h = ValidationHarness::new("exp079_scfa_production");
    ...
    h.check_bool(
        ...
        a0.abs() < MACHINE_EPSILON && p0.abs() < MACHINE_EPSILON && b0.abs() < MACHINE_EPSILON,
    );
    ...
    h.check_bool("all positive", a > 0.0 && p > 0.0 && b > 0.0);
