...
- **GAP-47 RESOLVED — Phase 2b signal collapse**: `s_nest_atomic` now tries `ctx.dispatch("nest.store")` + `ctx.dispatch("nest.commit")` via biomeOS orchestration before falling back to manual 5-step chain (Phases 3-7). SKIPs gracefully when orchestration unavailable, PASS when live biomeOS with signal graphs loaded. `HealthCompositionContext::dispatch()` wrapper added.
