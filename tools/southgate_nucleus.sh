...
#
# southgate_nucleus.sh — Deploy healthSpring's dual-tower NUCLEUS on southGate
#
...
#   ./tools/southgate_nucleus.sh status
...
start_primal() {
    ...
    local sock_name="${name}-${family}.sock"
    ...
    if check_health "$sock_path"; then
        ok "$name ($family) healthy — pid $pid"
        return 0
    else
...
do_start() {
    log "═══ southGate Dual-Tower NUCLEUS Start ═══"
    ...
    start_primal songbird "$FAMILY_A" "$SOCKET_DIR_A" "$SEED_A" || ((failed++))
    start_primal nestgate "$FAMILY_A" "$SOCKET_DIR_A" "$SEED_A" || ((failed++))
    start_primal rhizocrypt "$FAMILY_A" "$SOCKET_DIR_A" "$SEED_A" || ((failed++))
    start_primal loamspine "$FAMILY_A" "$SOCKET_DIR_A" "$SEED_A" || ((failed++))
    start_primal sweetgrass "$FAMILY_A" "$SOCKET_DIR_A" "$SEED_A" || ((failed++))
    start_primal biomeos "$FAMILY_A" "$SOCKET_DIR_A" "$SEED_A" || ((failed++))

    echo ""
    log "── Tower B (Analytics/Inference) ──"
    start_primal beardog "$FAMILY_B" "$SOCKET_DIR_B" "$SEED_B" || ((failed++))
    start_primal songbird "$FAMILY_B" "$SOCKET_DIR_B" "$SEED_B" || ((failed++))
    start_primal squirrel "$FAMILY_B" "$SOCKET_DIR_B" "$SEED_B" || ((failed++))
    start_primal nestgate "$FAMILY_B" "$SOCKET_DIR_B" "$SEED_B" || ((failed++))
    start_primal rhizocrypt "$FAMILY_B" "$SOCKET_DIR_B" "$SEED_B" || ((failed++))
    start_primal sweetgrass "$FAMILY_B" "$SOCKET_DIR_B" "$SEED_B" || ((failed++))
